...
public class asteroud325 implements Screen {
    Camera camera;
    ...
    public asteroud325(MyGdxGame myGdxGame) {
    ...
    public void show() {
        ...
        mapBody = new Array<>();
        map = new TmxMapLoader().load("asteroud325.tmx");
    ...
    public void render(float delta) {
        ...
        if (sprince.getBody().getPosition().y < 0) {
            myGdxGame.setScreen(myGdxGame.asteroud325);
        }
        if (sprince.getBody().getPosition().x > 600) {
            myGdxGame.setScreen(myGdxGame.king);
        }
